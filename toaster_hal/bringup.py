...
# Nahrad touhle verzi funkci cmd_pn532() v bringup.py.
# Zmeny oproti puvodni:
#   - "SPI = 0/0" -> "SPI = 0/1"  (puvodni text odpovidal HSU, ne SPI!)
#   - doplnena zminka o CS do diagnostiky
#   - volitelny --cs pro rucni chip select (docasny workaround bez overlay)
#   - poznamka o nutnem power cycle po prepnuti SEL prepinacu

def cmd_pn532(args) -> int:
    """Nejmensi mozny test PN532: precti verzi firmwaru."""
    from pn532 import PN532, PN532Error

    hal = get_hal("linux")
    try:
        spi = hal.spi_pn532()
        soft = getattr(spi, "soft_lsb", False)
        print(f"SPI: {PINOUT.spi_pn532_dev} @ {PINOUT.spi_pn532_hz} Hz, "
              f"LSB {'softwarove (ECSPI to neumi)' if soft else 'hardwarove'}")

        rst = None
        if args.reset:
            rst = hal.gpio_out("pn532_rst")
            print("pouzivam hardwarovy reset pres pn532_rst")

        cs = None
        if getattr(args, "cs", None):
            cs = hal.gpio_out(args.cs)
            print(f"rucni CS pres {args.cs} (workaround bez cs-gpios overlay)")

        dev = PN532(spi, reset=rst, cs=cs, debug=args.debug)
        ic, ver, rev, sup = dev.get_firmware_version()
        print(f"\nIC=0x{ic:02x}  firmware v{ver}.{rev}  support=0x{sup:02x}")
        if ic != 0x32:
            print("POZOR: IC neni 0x32, tohle nevypada na PN532")
            return 1
        print("OK - PN532 odpovida")

        if args.scan:
            dev.sam_configuration()
            print("\nprikladej karty, Ctrl-C ukonci")
            last = None
            while True:
                uid = dev.read_passive_target(timeout=0.5)
                if uid and uid != last:
                    print(f"  UID {uid.hex(' ')}  ({len(uid)} B)")
                last = uid
    except PN532Error as exc:
        print(f"\nCHYBA: {exc}")
        print("zkontroluj postupne:")
        print("  1. napajeni 3V3 (sviti PWR LED na modulu?)")
        print("  2. SEL prepinace: SEL0=0V, SEL1=3V3  (SPI = 0/1, NE 0/0!)")
        print("     po prepnuti nutny POWER CYCLE - PN532 cte konfiguraci")
        print("     rozhrani jen pri resetu")
        print("  3. propojeni MISO/MOSI/SCK/CS - pozor, X27 pin10=MISO,")
        print("     pin11=MOSI (snadno se prohodi)")
        print("  4. chip select: nativni ECSPI CS cvaka mezi slovy a PN532")
        print("     to rozhodi. Pust tools/spi_cs_check.py, pripadne")
        print("     nastav cs-gpios v device tree overlay.")
        return 1
    except KeyboardInterrupt:
        print("\nkonec")
        return 0
...
# --- a v main(), do parseru pro subcommand "pn532", pridej: ---
#
#     pn.add_argument("--cs", default=None,
#                     help="nazev GPIO pro rucni chip select "
#                          "(docasny workaround, kdyz nemas cs-gpios overlay)")
...
def main() -> int:
    ...
    pn = sub.add_parser("pn532")
    pn.add_argument("--scan", action="store_true", help="po verzi cti UID karet")
    pn.add_argument("--reset", action="store_true", help="hw reset pres pn532_rst")
    pn.add_argument("--debug", action="store_true", help="vypisuj syrove ramce")
    pn.add_argument("--cs", default=None, help="nazev GPIO pro rucni chip select ""(docasny workaround, kdyz nemas cs-gpios overlay)")
    pn.set_defaults(fn=cmd_pn532)
